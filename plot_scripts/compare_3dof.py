...
mpc_data_dir = os.path.join(data_dir, '3dof_mpc')
...
mpc_x    = np.load(os.path.join(mpc_data_dir, "mpc_x.npy"))

ol_u     = np.load(os.path.join(ol_data_dir, "u.npy"))
lqr_u    = np.load(os.path.join(lqr_data_dir, "u.npy"))
mpc_u    = np.load(os.path.join(mpc_data_dir, "mpc_u.npy"))

mpc_t = np.load(os.path.join(mpc_data_dir, "mpc_t.npy"))
...
plt.plot(mpc_t,mpc_x[:,0]-traj_pos[:,0])
...
plt.plot(mpc_t[:-1],mpc_u[:,0]/1000,'g',linestyle='dashed')
plt.ylabel(r"$T_x$ [kN]")
...
plt.plot(mpc_t[:-1],mpc_u[:,1]/1000,'g',linestyle='dashed')
plt.ylabel(r"$T_y$ [kN]")
...
plt.plot(mpc_t[:-1],mpc_u[:,2]/1000,'g',linestyle='dashed')
plt.legend(["Open Loop","LQR","MPC"])
plt.xlabel("t [s]")
plt.ylabel(r"$T_z$ [kN]")
